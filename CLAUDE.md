...
Plano de implementação de Landing page: `docs/superpowers/plans/2026-09-15-landing-page.md`

A Landing page está completa e no ar: página pública em `/` com Hero, features, "Como funciona" e CTA final, com redirecionamento automático para `/painel` quando o usuário já está logado.
...
- **`Button`/`DialogTrigger` como link ou disparador de outro componente:** usar sempre a prop `render` (ex.: `<Button render={<a href={link} target="_blank" rel="noreferrer" />}>Texto</Button>` ou `<DialogTrigger render={<Button variant="destructive" />}>Excluir</DialogTrigger>`), nunca `asChild` nem `onClick` + `window.open`. As primitivas `@base-ui/react` deste projeto não suportam `asChild`, e `onClick`/`window.open` faz o elemento perder a semântica de link (`role="link"`, abrir em nova aba pelo navegador, etc). Um ciclo de duas tentativas erradas (`asChild`, depois `onClick`) já quebrou 4 testes numa sessão anterior antes de restaurar o padrão `render` original — ver commit `914bdd0`.
